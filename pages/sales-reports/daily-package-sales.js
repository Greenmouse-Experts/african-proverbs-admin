...
import { getDailyPackageSales } from "@/services/salesReport";
...
const createData = ({
  ...
  return { package_name, total_count, sales_price, total_sales };
};

const rowsData = (rdata) => {
  //console.log("Date Data", rdata);
  return rdata?.map((data) => {
    const { package_name, total_count, sales_price, total_sales } = data || {};
    return createData({
      package_name,
      total_count,
      sales_price,
...
const CustomizedTables = () => {
  ...
  const sendRequest = async () => {
    ...
    const filterResults = await getDailyPackageSales({
      start_date: formatDate(startDate),
      end_date: formatDate(endDate),
      packageid: selectedPackageId || "All",
    });
  ...
  useEffect(() => {
    const getReports = async () => {
      console.log("Obi jagaban");
      const reportsData = await getDailyPackageSales({
          ...
          <Paper className={classes.root} ref={componentRef}>
            <TableContainer className={classes.container}>
              <Table stickyHeader aria-label="sticky table">
                <TableHead>
                  <TableRow>
                    {dailyPackageColumns.map((column) => (
                      <TableCell
                        key={column.id}
                        align={column.align}
                        style={{
