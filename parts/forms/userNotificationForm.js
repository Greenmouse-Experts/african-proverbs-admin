...
import { sendPostNotification, sendUserNotification } from '@/services/subscribersService';
...
const onSubmit = (data) => {
    ...
    "email":checked?'': `${email}`
        ...
        sendUserNotification(payload).then(()=>
    ...
    />
                        ...
                        <button disabled={buttonDisabled} type='submit' className="btn col-md-12 btn-purple waves-effect waves-light mr-3">{isLoading ? (
                            <CircularProgress size={16} color="white" />):null}  <span>Send Push Notification</span> </button>                        </div>
                        <br />
                    </div>
                </form>
