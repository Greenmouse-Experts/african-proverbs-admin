...
const PublisherReviews = ({ proverb, user }) => {
    ...
    return (
        <>
            <div class="row">
                <div class="col-lg-12">
                    {
                        user && (checkPermission(user.roles, 'Publisher')) | (checkPermission(user.roles, 'Reviewer')) | (checkPermission(user.roles, 'Admin')) | (checkPermission(user.roles, 'SuperAdmin')) ?
                            <div class="col-lg-3 mt-2 mb-2">
                                <div class="widget">
                                    <div class="widget-body">
                                        <a href="#" data-toggle="modal" onClick={()=>openModal()} data-target="#add-Publishercomment" class="btn btn-lg btn-purple font-16 btn-block waves-effect waves-light">
                                            <i class="fa fa-plus mr-1"></i> Create New Comment
                                        </a>
                                        <CommentForm closeModal={closeModal} openModal={openModal} open={open} state="PUBLISH" requestType="Create" proverb={proverb}/>
                                    </div>
                                </div>
                            </div>
                            : null
                    ...
                    <div class="row">
                        {
                            proverb && proverb.proverbReview.length > 0 ?
                                proverb.proverbReview.map(review => review.state==="PUBLISH" && (
                                    <div key={review.id} class="col-md-4">
                                        ...
                                        >
                                            {
                                                // user && user.role == 'Reviewer' | user.role == 'Admin' | user.role == 'SuperAdmin' ?
                                                user && (checkPermission(user.roles, 'Publisher')) | (checkPermission(user.roles, 'Reviewer')) | (checkPermission(user.roles, 'Admin')) | (checkPermission(user.roles, 'SuperAdmin')) ?
                                                    <>
                                                        <li class="list-inline-item">
                                                            <a onClick={() => openModalEdit(review)} class="icon circle-icon text-info"><i class="mdi mdi-circle-edit-outline"></i></a>
                                                        </li>
                                ...
                                <div class="col-md-6 offset-md-4">
                                    <p>No Publisher's Comments added yet</p>
                                </div>
            ...
            <CommentForm closeModal={closeModalEdit} openModal={openModalEdit} open={openEdit} commentData={data} state="PUBLISH" requestType="Edit" proverb={proverb}/>
            ...
            {/* <div class="modal fade none-border" id="add-Publishercomment">
